...
public func Launch(int iAngle, int iSpeed, int iDist, int iSize, int iTrail, int iDmg, int iDmgPrec, int iGlowSize, int iAPrec, int iReflections)
{
  ...
  SetR(+iAngle);
  //verschwinden bei am boden liegender granate verhindern
  if(GBackSolid(GetX(),GetY()))
  {
  SetPosition(GetX()+GetXDir()/10,GetY()+GetYDir()/10,this);
  }
  ...
  if(GetOCF(pObj)&OCF_Living)
	if(pObj->~IsRealHurtable())
	{
		if(pObj->~IsCrawling())
		{
			...
			//Brusttreffer
			if(GetX()> GetXTop(pObj) + GetDefCoreVal("Width",0,GetID(pObj))/3 && GetY()< GetXTop(pObj) + GetDefCoreVal("Width",0,GetID(pObj))/3*2)
			{
				var r = RandomX(1,100);
