...
public func HandX()    			{ return PostureValue(3000,-1000);}
public func HandY()    			{ return PostureValue(-1000,1500);}
...
public func FMData1(int data)
{
	...
	if(data == FM_WeaponSound)return "SMAW_s";
	return Default(data);
}
...
public func FMData2(int data)
{
	...
	if(data == FM_AmmoID)    	return MIAM;
	if(data == FM_AmmoLoad)  	return 1;
	if(data == FM_Reload)    	return 12*36;
	if(data == FM_Recharge)  	return 0;
	if(data == FM_Aim)       	return 1;
	if(data == FM_Damage)    	return 50;
	if(data == FM_WeaponSound)return "SMAW_s";
